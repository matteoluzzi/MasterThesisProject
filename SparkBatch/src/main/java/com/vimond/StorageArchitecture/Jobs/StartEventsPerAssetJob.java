...
public class StartEventsPerAssetJob extends WorkingJob<EventInfo>
{
	private static final long serialVersionUID = 7797197320996513332L;

	public StartEventsPerAssetJob(JavaRDD<EventInfo> inputDataset, DateTime timestamp, String timewindow)
	{
	...
	{
		JavaPairRDD<String, Integer> mappedRdd = this.inputDataset.mapToPair(t -> new Tuple2<String, Integer>(t.getAssetName(), 1));
		...
		JavaRDD<String> result = mappedRdd.mapPartitions(new FlatMapFunction<Iterator<Tuple2<String, Integer>>, String>()
		{
			...
			public Iterable<String> call(Iterator<Tuple2<String, Integer>> tuples) throws Exception
			{
				...
				while (tuples.hasNext())
				{
					...
					counterByAsset.eventName = "StartEventPerAsset";
