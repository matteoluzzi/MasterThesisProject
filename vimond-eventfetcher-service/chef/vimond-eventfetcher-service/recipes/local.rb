...
service_jar_path = "#{repo}/#{groupid}/#{artifact_name}/#{version}/#{artifact_name}-#{version}.jar"

service_folder = node[:vimond_eventfetcher_service][:service_folder]
local_service_file = "#{service_folder}/#{service_jar_path.split('/')[-1]}"
...
execute "copy-jar" do
  user user
  group group
  command "cp #{service_jar_path} #{local_service_file}"
  action :run
end
